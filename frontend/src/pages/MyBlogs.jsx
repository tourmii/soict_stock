      ...
      <div className="container blogs-shell">
        <div className="blogs-header">
          <div>
            ...
            <h2>Draft, Publish, and Manage Posts</h2>
        ...
        <div className="my-blog-list">
          {posts.map((post) => (
            <article key={post.id} className="my-blog-row">
              <div>
                ...
                {post.status === 'published' && (
                  <Link to={`/blogs/${post.slug}`} className="blog-read-link">View public post</Link>
                )}
              </div>
              <div className="my-blog-row__actions">
                <Link to={`/my-blogs/${post.id}/edit`} className="btn btn-outline btn-sm">Edit</Link>
                {post.status === 'published' && (
                  <button className="btn btn-outline btn-sm" onClick={() => handleArchive(post.id)}>Archive</button>
                )}
                {post.status !== 'published' && (
                  <button className="btn btn-outline btn-sm blog-danger-btn" onClick={() => handleDelete(post)}>Delete</button>
                )}
              </div>
            </article>
          ))}
