...
export default function ContestStockChart() {
  ...
  const displayBars = useMemo(() => {
    const maxBars = { '15m': 200, '1H': 200, '4H': 200, '1D': 365, '1W': 104, '1M': 60 };
        ...
        <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
          <select value={selectedTicker} onChange={(e)=>setSelectedTicker(e.target.value)} className="input select" style={{width:'auto',padding:'6px 30px 6px 10px',fontWeight:600,fontSize:'var(--text-sm)'}}>
            {allowedTickers.map((ticker) => {
              const s = STOCKS.find(stock => stock.ticker === ticker) || currentContest?.customStocks?.find(stock => stock.ticker === ticker);
              if (!s) return null;
              return <option key={s.ticker} value={s.ticker}>{s.ticker} — {s.name}</option>;
            })}
          </select>
          <div>
